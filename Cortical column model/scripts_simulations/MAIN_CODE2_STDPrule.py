...
# This cortical column model has PLASTIC WEIGHTS between excitatory neurons (STDP plastic rule is used).

# In this code example half of the excitatroy neurons in layer 4 are receiving an input of 30pA.
# This specific choice produce the data of Fig. 4B.


# Note: 
# This program has a long computation time (aroun 6h)
...
a='../simulations_data/Spikes_stdp_stopped_30pA' #This is the folder that will contain the spikes files
#b='../simulations_data/weights_track_changes' #This is the folder that will contain the weights files (if I want to save them)

t_stop=55
runtime = (t_stop*1000.0) * ms #How long you want the simulation to be
dt_sim=0.1 #ms
        ...
        for k in range(len(targets)):
            ...
            if s_cell_type==0: # sendind is excitatory neuron
                ...
                if t_cell_type==0: # target is excitatory neuron
                    ...
                    #STDP rule is applied
                    conn= Synapses(pops[s_layer][s_cell_type],pops[t_layer][t_cell_type],model=eqs_ampa[s_layer],
                                   on_pre='''
                                   s_AMPA+= 1
                                   apre += dApre
                                   w_AMPA = clip(w_AMPA + apost, 0, wmax) #w_AMPA=w_AMPA+apost
                                   ''',
                                   on_post='''
                                   apost += dApost
                                   w_AMPA = clip(w_AMPA + apre, 0, wmax) #w_AMPA=w_AMPA + apre
                                   ''', method='euler')
                ...
                if t_cell_type!=0: # target is inhibitory neuron (no STDP in inhibitory neurons!)
...
# I store the state of the network in a file
net.store(filename='simulations_data/network_state')
